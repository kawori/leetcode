...
impl Solution {
    pub fn subdomain_visits(cpdomains: Vec<String>) -> Vec<String> {
        ...
        for visit in cpdomains {
            if let Some(space) = visit.find(' ') {
                let visit_cnt: i32 = visit[..space].parse().unwrap();
                let domain = &visit[space + 1..];
                let mut subs: Vec<&str> = Vec::new();
                subs.push(domain);
                if let Some(dot) = domain.rfind('.') {
                    subs.push(&domain[dot + 1..]);
                    let left = &domain[..dot];
                ...
                for sub in subs {
                    if let Some(x) = map.get_mut(sub) {
                        *x += visit_cnt;
                    } else {
                        map.insert(String::from(sub), visit_cnt);
                    }
                }
            }
        }
...
fn main() {
    ...
    println!(
        "{:?} -> {:?}",
        cpdomains.clone(),
        Solution::subdomain_visits(cpdomains)
    );
}
