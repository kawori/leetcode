...
impl Solution {
    pub fn judge_circle(moves: String) -> bool {
        let nl = moves.chars().filter(|c| *c == 'L').count();
        let nr = moves.chars().filter(|c| *c == 'R').count();
        let nu = moves.chars().filter(|c| *c == 'U').count();
        let nd = moves.chars().filter(|c| *c == 'D').count();
        nl == nr && nu == nd
    }
}
...
fn main() {
    ...
    let mc = moves.clone();
    println!("{} -> {}", mc, Solution::judge_circle(moves));
}
